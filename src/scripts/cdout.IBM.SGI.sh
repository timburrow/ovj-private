"@(#)cdout.IBM.SGI.sh 22.1 03/24/08 2003-2004 ."
#!/bin/sh
...
iProcLinuxFam="			\
	...
	-C $lnxproglib_dir/atproc	Atproc		\
        -C $lnxproglib_dir/psg		seqgenmake	\
	-C $vcommondir spincad	\
        "
...
IbmBinX2Tar="			\
	...
	bin/pulsetool		\
	bin/pulsechild		\
	-C $vcommondir/tape_ibm app-defaults/Vnmr		\
	-C $vcommondir/tape_ibm app-defaults/PulseTool	\
	-C $vcommondir/tape_ibm app-defaults/Status	\
	-C $vcommondir/tape_ibm app-defaults/Enter		\
	"
...
IbmBinAcq2Tar="			\
	...
	bin/editdevices		\
	bin/send2Vnmr           \
        -C $vcommondir/tape_ibm app-defaults/Config      \
	"
...
SgiBinX2Tar="			\
	...
	bin/pulsechild		\
	bin/pulsetool		\
	-C $vcommondir/tape_sgi app-defaults/Vnmr		\
	-C $vcommondir/tape_sgi app-defaults/PulseTool	\
	-C $vcommondir/tape_sgi app-defaults/Status	\
	-C $vcommondir/tape_sgi app-defaults/Enter		\
	-C $vcommondir/tape_sgi app-defaults/Dg		\
	"
...
SgiBinAcq2Tar="			\
	...
	bin/editdevices		\
	bin/send2Vnmr           \
        -C $vcommondir/tape_sgi app-defaults/Config      \
	"
#  removed from above list
#	bin/editdevices	
#	bin/portrait
...
BinPFGCP2Tar="seqlib/wet1D	\
	...
	seqlib/wetgHSQC		\
	"
iBinPFGLinux2Tar="$BinPFGCP2Tar $BinPFG2Tar seqlib/wet1d"
...
#-- directories from IMAGE2 fro HEI sequences
ComIMAGE22Tar="			\
	maclib			\
	"
...
#	TOCfile:   sol/inova.sol, /sol/unity.sol, ibm/ibm.ibm ..., these files contain
#					a list of needed sofware for particular sytem
make_TOC()
{
   ( #usage: make_TOC  <tarfile>  <dirname>  <category>  <TOCfile TOCfile ...>
     tfile=$1
...
echo "ibm_answer	= $ibm_answer +++++++++++++"
echo "sgi_answer	= $sgi_answer +++++++++++++"
...
#---------------------------------------------------------------------------
   ...
   tar -cf - $Tcl2Tar | (cd $dest_dir_code/tmp; tar xfBp -)
   cd $dest_dir_code/tmp
   rm -f tcl/bin/vnmr* tcl/bin/spingen
...
# tar psglib
   ...
   (cd $vcommondir; cp -rp psglib $dest_dir_code/tmp)
   chmod 755   ./psglib
   chmod 644   ./psglib/*
   tar cf $dest_dir_code/$Inova/psglib.tar *
   make_TOC psglib.tar $Inova "VNMR" ibm/ibm.ibm	\
   				     sgi/sgi.sgi
...
#
   ...
   cd $dest_dir_code/tmp
   chmod  -R  755 ./lib
   tar -cf $dest_dir_code/$Linux/tclliblnx.tar lib
...
############################################################
############################################################
#   ATTENTION: Be awared of the LoadP11 switch below
############################################################

if [ x$LoadP11 = "xy" ]
then
    echo ""
    nnl_echo "Skipping PART VII -- GEMINI 2000 FILES -- for Part11" | tee -a $log_fln
    echo ""
    nnl_echo "Skipping PART VIIb -- MERCURY FILES -- for Part11" | tee -a $log_fln
else
...
#Linux: kpsg libraries into /vnmr/lib directory
   ...
   chmod 644      ./lib/x_ps.o
   mkdir acqbin
   cp /vobj/lnx/proglib/psg/seqgenmake acqbin
   chmod -R 755   ./acqbin
...
#Linux: Inova psg C files library and Inova binary seqlib
   log_this "Linux, Tarring psgclib.tar     for : "

   mkdir -p $dest_dir_code/tmp/psglib
   cd $lnxproglib_dir/psglib
   for cfile in $iBinSeq2Tar
   do
   ...
   cd $dest_dir_code/tmp
   setperms psglib 755 644 755
   tar -cf $dest_dir_code/$Linux/psgclib.tar psglib
   make_TOC psgclib.tar $Linux "VNMR" rht/inova.rht


   log_this "Linux, Tarring seqlib.tar     for : "
   cd $lnxproglib_dir
...
#Linux: psg C files library for Mercury +
   log_this " Tarring kpsgclib.tar     for : "
...
# tar kpsglib 
   cd $dest_dir_code/tmp
   log_this " Tarring kseqlib files     for : "
...
# tar psg to psg
   ...
   make_toc $Inova "VNMR" ibm/ibm.ibm
   make_toc $Inova "VNMR" sgi/sgi.sgi
   rm -rf $dest_dir_code/tmp/*
...
# tar some common text files into one tar file
  ...
  then
   ...
   make_toc $Irix "VNMR" sgi/sgi.sgi
   rm -rf $dest_dir_code/tmp/*
...
# tar spincad tuff
  ...
  then
   ...
   cp -r $vcommondir/spincad .
   cp  $sourcedir/systcl/sgi/spincad.tbc     bin/spincad
   cp  $sourcedir/systcl/sgi/spingen.tbc     tcl/bin/spingen
...
#---------------------------------------------------------------------------
   ...
   cd $lnxproglib_dir
   tar -cf - $iBinPFGLinux2Tar | (cd $dest_dir_code/tmp; tar xfBp -)
   cd $dest_dir_code/tmp
   chmod -R 755   ./seqlib
...
# tar the INOVA LCNMR seqlib and bin
  ...
  then
   	cd $solobjdir
   	log_this " Tarring LCNMR seqlib and bin	  for: "
...
# tar Backproj's binary files
  ...
  then
   ...
   cd $solobjdir
   echo "" | tee -a $log_fln
   nnl_echo " Tarring Backproj binary  fro : " | tee -a $log_fln
...
#---------------------------------------------------------------------
# Linux, Inova  DOSY files
  if [ x$password_answer = "xy" ]
  then
       log_this "Linux, Tarring DOSY files          for : "

       cd $dest_dir_code/tmp
       (cd $vcommondir/DOSY; cp -rp * $dest_dir_code/tmp)

       (cd $lnxproglib_dir/psglib; cp -f Dbppste.c Dbppsteinept.c DgcsteSL.c \
                   Dgcstecosy.c Dgcstehmqc.c Doneshot.c $dest_dir_code/tmp/psglib)
       (cd $lnxproglib_dir/seqlib; cp -f Dbppste Dbppsteinept DgcsteSL Dgcstecosy \
                                    Dgcstehmqc Doneshot $dest_dir_code/tmp/seqlib)
       ...
       tar -cf - * | $Encodedir/encode $DOSY_password >$dest_dir_code/$Linux/i_dosy.pwd
       make_TOC i_dosy.pwd $Linux "DOSY" rht/inova.opt
 else
       log_this "Linux, Skipping DOSY files          for : "
       make_TOC i_dosy.pwd $Linux "DOSY" rht/inova.opt
 fi
...
#
#---------------------------------------------------------------------
# Linux, tar DOSY files
  if [ x$password_answer = "xy" ]
  then
      cd $dest_dir_code/tmp
      echo "" | tee -a $log_fln
      nnl_echo "Linux, Tarring DOSY files       for : " | tee -a $log_fln
      (cd $vcommondir/DOSY; cp -rp * $dest_dir_code/tmp)
      rm -rf ./user_templates
      chmod 755   ./*
      chmod 644   ./*/*
      ...
      for file in $filelist
      do
          cp -p /vobj/lnx/proglib/kseqlib/$file seqlib
      done
      chmod 755 ./seqlib/*
      ...
      tar -cf - * | $Encodedir/encode $DOSY_password >$dest_dir_code/$Linux/ldosy.pwd
      set_size_name $Linux ldosy.pwd
      make_toc $Linux "DOSY" rht/mercplus.opt
      rm -rf $dest_dir_code/tmp/*
...
# tar 768 AS files
  ...
  then
   ...
   cd $dest_dir_code/tmp
   echo "" | tee -a $log_fln
   nnl_echo " Tarring 768 AS files	  for : " | tee -a $log_fln
   (cd $vcommondir/768AS; cp -rp * $dest_dir_code/tmp)
#   chmod 755   ./*
#   chmod 644   ./*/*
#   chmod 755   ./bin/*
#   chmod 755   ./tcl/bin
...
#   chmod 644   ./asm/tcl/*
   ...
   set_size_name $Unity/$Inova 768AS.pwd
   make_toc $Unity/$Inova "768_AS" sol/inova.opt

 else

   echo "" | tee -a $log_fln
   nnl_echo " Skipping 768 AS files	  for : " | tee -a $log_fln
   ...
   cat $dest_dir_code/rht/mercplus.opt | tee -a $log_fln

#---------------------------------------------------------------------
#Linux, tar 768 AS files
  if [ x$password_answer = "xy" ]
  then
       log_this "Linux, Tarring 768 AS files        for : "
       cd $dest_dir_code/tmp
       (cd $vcommondir/768AS; cp -rp * $dest_dir_code/tmp)
       ...
       log_this "Linux, Skipping 768 AS files       for : "
       make_TOC 768AS.pwd $Linux "768_AS" rht/inova.opt
 fi
...
#   echo "VNMR for SGI     $SGI_password"   >> passwords
   ...
   echo "768 AS            $AS768_password" >> passwords
