...
do
  ...
  # Interix tar list includes directories but without the back slash  i.e. 'bin' not 'bin/'
  # Thus it's not stripped by patchinstall and there is not a simple effective way of doing this.
  # Thus here we check if the 'file' is a directory, if it is then skip it. 
  ...
  then
      mv $patch_process_dir/$File $vnmrsystem/$File
      unzip -oq $vnmrsystem/$File -d $vnmrsystem
      rm -f $vnmrsystem/$File
...
then
    if [ -f $vnmrsystem/templates/vnmrj/interface/MainMenuFile.xml.p11 ]
    then
      cp $vnmrsystem/templates/vnmrj/interface/MainMenuFile.xml.p11 $vnmrsystem/templates/vnmrj/interface/MainMenuFile.xml
    fi
fi
